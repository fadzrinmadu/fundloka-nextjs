...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    api
      .get<PaginatedApiResponse<Campaign[]>>(`/api/v1/campaigns?${params.toString()}`)
      .then((response) => {
        if (cancelled) return;
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
          ...
          <div>
            <Link
              ...
              className="bg-orange-button hover:bg-green-button text-white font-bold py-4 px-4 rounded inline-flex items-center justify-center w-full sm:w-auto"
            >
              + Create Campaign
            </Link>
          </div>
        ...
        <div className="block mb-2">
          {loading ? (
            ...
            <div className="text-center text-gray-500 py-16">
              {search ? `No projects found for "${search}".` : 'You have no projects yet.'}
            </div>
          ) : (
